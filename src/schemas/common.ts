import { z } from "zod";
import { DEFAULT_LIMIT, DEFAULT_MAX_PAGES, MAX_PAGES, MAX_WITHINGS_LIMIT } from "../constants.js";
...
export const IdInputSchema = z.object({
  id: z.union([z.string().min(1), z.number().int().positive()]).describe("Withings resource id."),
  privacy_mode: PrivacyModeSchema,
  response_format: ResponseFormatSchema
}).strict();
...
export const AuthUrlInputSchema = z.object({
  ...
  scopes: z.array(z.string()).optional().describe("Optional scope override. Defaults to read-only Withings scopes used by this server."),
  response_format: ResponseFormatSchema
}).strict();
...
export const WellnessContextInputSchema = z.object({
  days: z.number().int().min(1).max(30).default(7).describe("Lookback window for normalized Withings wellness context."),
  timezone: z.string().min(1).max(80).default("UTC").describe("IANA timezone used only for display, e.g. America/New_York."),
...
export const ConnectionStatusOutputSchema = z.object({
  ok: z.boolean(),
  ready_for_withings_api: z.boolean(),
  client: AgentClientSchema.optional(),
  ...
  client_checks: z.object({
    hermes: z.object({
      ...
      withings_server_configured: z.boolean(),
